  ...
  return (
    <section className="bg-ivory-warm section-space">
      <div className="shell">
        <SectionHeader
          title="지금 많이 보는 콘텐츠"
          description="학부모가 최근 먼저 눌러 보는 콘텐츠를 정리해 두었습니다."
          href="/contents"
        />
        ...
        {items.length > 0 ? (
          <div className="overflow-hidden rounded-[32px] border border-black/6 bg-white">
            {items.map((content, index) => (
              <Link
                ...
                className="relative grid gap-4 border-b border-black/6 px-5 py-5 transition hover:bg-ivory sm:grid-cols-[72px_1fr] sm:px-7 last:border-b-0"
              >
          ...
          <EmptyState
            ...
            description="조금만 더 운영하면 학부모가 먼저 찾는 흐름이 여기에 정리됩니다."
          />
        )}
      </div>
    </section>
